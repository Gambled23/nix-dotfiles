...
HAYASE_LATEST_VER="$(curl -s ${GITHUB_TOKEN:+-u ":$GITHUB_TOKEN"} "https://api.github.com/repos/hayase-app/ui/releases/latest" | jq -r .tag_name | sed 's/^v//')"
...
HAYASE_LINUX_HASH="$(get_hash "https://github.com/hayase-app/ui/releases/download/v${HAYASE_LATEST_VER}/linux-hayase-${HAYASE_LATEST_VER}-linux.AppImage")"
HAYASE_DARWIN_HASH="$(get_hash "https://github.com/hayase-app/ui/releases/download/v${HAYASE_LATEST_VER}/mac-hayase-${HAYASE_LATEST_VER}-mac.zip")"
